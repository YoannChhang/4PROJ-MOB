...
export const useRouteNavigation = (
  ...
  const [announcementsMadeForStep, setAnnouncementsMadeForStep] = useState<Set<string>>(new Set());
  ...
  const speakInstruction = useCallback((instruction: string, isManeuverChange: boolean = false) => {
    ...
    if (isNavigating && selectedRoute !== routeRef.current) {
      ...
      if (selectedRoute) {
        ...
        if (selectedRoute.legs[0]?.steps[0]) {
          const initialManeuverInstruction = selectedRoute.legs[0].steps[0].maneuver.instruction;
          ...
          if (firstStepVIs && firstStepVIs.length > 0 && firstStepVIs[0].distanceAlongGeometry === 0) {
  ...
  const startLocationTracking = async () => {
    ...
    locationSubscription.current = await Location.watchPositionAsync(
      ...
      (location) => setLiveUserLocation([location.coords.longitude, location.coords.latitude])
    );
  };
  ...
  const updateTraveledRoute = (userLocation: Coordinate) => {
    setTraveledCoords((prev) => {
      ...
      if (lastPoint && turf.distance(turf.point(lastPoint), turf.point(userLocation), { units: "meters" }) < 5) return prev;
  ...
  const checkAndAnnounceVoiceInstructions = useCallback((
    ...
    const sortedVIs = [...voiceInstructions].sort((a, b) => a.distanceAlongGeometry - b.distanceAlongGeometry);
    ...
    for (const vi of sortedVIs) {
      ...
      if (progressInCurrentStep >= (vi.distanceAlongGeometry - ACTIVATION_BUFFER_METERS) &&
          progressInCurrentStep < vi.distanceAlongGeometry + 10 ) {
        console.log(`VI Activated: "${vi.announcement}". User at ${progressInCurrentStep.toFixed(0)}m in step. VI point at ${vi.distanceAlongGeometry.toFixed(0)}m.`);
        speakInstruction(vi.announcement); // This will also update displayedInstruction
        setAnnouncementsMadeForStep((prev) => new Set(prev).add(announcementKey));
        return;
      }
    }
  }, [speakInstruction, announcementsMadeForStep, currentStepIndex]);

  const checkRouteProgress = useCallback((userLocation: Coordinate) => {
    ...
    try {
      ...
      const { distance: distanceFromRoute, location: progressAlongEntireRoute } = findNearestPointOnRoute(selectedRoute, userLocation);

      if (distanceFromRoute > OFF_ROUTE_THRESHOLD) {
        offRouteCountRef.current += 1;
        if (offRouteCountRef.current >= (OFF_ROUTE_CONFIRMATION_COUNT || 3)) {
          offRouteCountRef.current = 0;
          if (onOffRoute) onOffRoute(userLocation);
        }
        return;
      } else {
        offRouteCountRef.current = 0;
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (progressAlongEntireRoute >= cumulativeDistanceAtStartOfCurrentStep && progressAlongEntireRoute < cumulativeDistanceAtStartOfCurrentStep + stepDistance) {
          newStepIndex = i;
          break;
        }
      ...
      if (newStepIndex !== currentStepIndex) {
        ...
        const newManeuverInstruction = steps[newStepIndex].maneuver.instruction;
      ...
      const progressInCurrentStep = progressAlongEntireRoute - cumulativeDistanceAtStartOfCurrentStep;

      if (newStepIndex < steps.length - 1) {
        const nextStepObject = steps[newStepIndex + 1];
        const nextManeuverLocation = nextStepObject.maneuver.location as Coordinate;
        const distToNextManeuverPoint = calculateDistanceInMeters(userLocation, nextManeuverLocation);
        setDistanceToNextManeuver(distToNextManeuverPoint);
        checkAndAnnounceVoiceInstructions(currentStepObject, progressInCurrentStep);
      } else {
        const destinationCoords = selectedRoute.geometry.coordinates[selectedRoute.geometry.coordinates.length - 1] as Coordinate;
        const distanceToDestination = calculateDistanceInMeters(userLocation, destinationCoords);
      ...
      const updatedRemainingDistance = Math.max(0, selectedRoute.distance - progressAlongEntireRoute);
      const progressPercentage = selectedRoute.distance > 0 ? progressAlongEntireRoute / selectedRoute.distance : 0;
      const updatedRemainingDuration = selectedRoute.duration * (1 - progressPercentage);
      setRemainingDistance(updatedRemainingDistance);
      setRemainingDuration(updatedRemainingDuration);
      setEstimatedArrival(new Date(Date.now() + updatedRemainingDuration * 1000));
    } catch (err) {
      console.error("Error checking route progress:", err);
    }
  }, [selectedRoute, isNavigating, currentStepIndex, onOffRoute, onArrive, speakInstruction, checkAndAnnounceVoiceInstructions]);
  ...
  const updateNavigationMetrics = useCallback((freshRoute: Route) => {
    if (!freshRoute || !selectedRoute || !liveUserLocation) return;
    const { location: progressAlongOldRoute } = findNearestPointOnRoute(selectedRoute, liveUserLocation);
    const progressPercentage = selectedRoute.distance > 0 ? progressAlongOldRoute / selectedRoute.distance : 0;
    const freshRemainingDistance = freshRoute.distance * (1 - progressPercentage);
    const freshRemainingDuration = freshRoute.duration * (1 - progressPercentage);
    setRemainingDistance(Math.max(0, freshRemainingDistance));
    setRemainingDuration(Math.max(0, freshRemainingDuration));
    setEstimatedArrival(new Date(Date.now() + Math.max(0, freshRemainingDuration) * 1000));
  ...
  return {
    isNavigating, setIsNavigating, liveUserLocation, traveledCoords,
    // currentInstruction, // No longer explicitly needed if displayedInstruction is used
    displayedInstruction, // NEW: Expose this
    distanceToNextManeuver, currentStepIndex, remainingDistance, remainingDuration,
    estimatedArrival, startNavigation, stopNavigation, updateNavigationMetrics,
  };
};