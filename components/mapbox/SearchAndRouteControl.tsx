...
const SearchAndRouteControl: React.FC<SearchAndRouteControlProps> = ({
  ...
  const handleSelectSearchResult = useCallback(async (feature: any) => {
  ...
  }, [userLocation, onDestinationSelected, onRouteSelected, userData?.preferences]);
  ...
  const handleStartNavigation = useCallback(async () => {
    ...
    if (Platform.OS === 'ios') {
      ...
      if (status !== 'granted') {
        ttsManager.speak('Pour une navigation optimale, veuillez autoriser la localisation en arrière-plan');
      }
    }
          ...
          <ScrollView style={styles.routesList}>
            {loading ? (
              <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Recherche des itinéraires...</Text>
              </View>
            ) : (
                ...
                >
                  ...
                  <View style={styles.routeTypeContainer}>
                    <Text style={styles.routeType}>
                      {route.weight_name === 'auto' ? 'Recommandé' : 
                       route.weight_name === 'shortest' ? 'Le plus court' : 
                       'Le plus rapide'}
                    </Text>
                    
                    {route.legs[0]?.steps.some(step => 
                      step.maneuver.instruction.toLowerCase().includes('péage')
                    ) && (
                      <View style={styles.routeBadge}>
                        <Text style={styles.routeBadgeText}>Péage</Text>
                      </View>
                    )}
                  </View>
                </TouchableOpacity>
